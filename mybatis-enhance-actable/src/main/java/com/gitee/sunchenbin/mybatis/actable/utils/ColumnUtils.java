...
public class ColumnUtils {
    ...
    /**
     * 获取枚举类型中带有@EnumValue注解的字段类型
     *
     * @param enumClass 枚举类
     * @return 带有@EnumValue注解的字段类型，如果没有找到则返回null
     */
    private static Class<?> getEnumValueFieldType(Class<?> enumClass) {
        if (!enumClass.isEnum()) {
            return null;
        }
    ...
    public static MySqlTypeAndLength getMySqlTypeAndLength(Field field, Class<?> clasz) {
        ...
        if (type != null && type.value() != null && type.value() != MySqlTypeConstant.DEFAULT) {
            return buildMySqlTypeAndLength(field, type.value().toString().toLowerCase(), type.length(),
                    type.decimalLength());
        }
        if (type != null && columnCommon != null && type.value() != null && type.value() != MySqlTypeConstant.DEFAULT) {
            return buildMySqlTypeAndLength(field, type.value().toString().toLowerCase(), columnCommon.length(),
                    columnCommon.scale());
        }
        // 类型为空根据字段类型去默认匹配类型
        MySqlTypeConstant mysqlType = JavaToMysqlType.javaToMysqlTypeMap.get(field.getGenericType().toString());

        // 如果基本类型映射中没有找到，检查是否为枚举类型
        if (mysqlType == null && field.getType().isEnum()) {
            Class<?> enumValueFieldType = getEnumValueFieldType(field.getType());
            if (enumValueFieldType != null) {
                // 使用枚举中@EnumValue注解字段的类型进行映射
                mysqlType = JavaToMysqlType.javaToMysqlTypeMap.get("class " + enumValueFieldType.getName());
            }
        }
    ...
    private static MySqlTypeAndLength buildMySqlTypeAndLength(Field field, String type, int length, int decimalLength) {
        ...
        if (mySqlTypeAndLength == null) {
            throw new RuntimeException("字段名：" + field.getName() + "使用的" + type + "类型，没有配置对应的MySqlTypeConstant" +
                    "，只支持创建MySqlTypeConstant中类型的字段，异常抛出！");
        }
